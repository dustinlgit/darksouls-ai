import movement.actions as actions
import time
...
class ds3Env(gym.Env):
    ...
    def _get_game_state(self):
        ...
        try:
            ...
            return {
                'player_hp': self.ds3.read_int(player_curr_hp),
                'player_max_hp': self.ds3.read_int(player_max_hp),
                'player_sp': self.ds3.read_int(player_curr_sp),
                'player_max_sp': self.ds3.read_int(player_max_sp),
                'boss_hp': self.ds3.read_int(boss_curr_hp),
                'boss_max_hp': self.ds3.read_int(boss_max_hp)
            }
        except Exception as e:
    ...
    def _get_observation(self):
        ...
        # Normalize stats
        player_hp_ratio = state['player_hp'] / max(state['player_max_hp'], 1)
        player_sp_ratio = state['player_sp'] / max(state['player_max_sp'], 1)
        boss_hp_ratio = state['boss_hp'] / max(state['boss_max_hp'], 1)
